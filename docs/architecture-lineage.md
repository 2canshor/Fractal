...
[Fractal Blueprint](blueprint.md) is the only current workflow architecture. Git history preserves earlier designs; they are evidence, not a second active rulebook.
...
- `$` Deuteragonist: retain the core concept while implementation may change.
- `^` Extra: supporting Infrastructure that stays outside product emphasis.
- `¢` Prop: reusable Method without lifecycle or change authority.
- `%` Principle: asymptotic direction that cannot be claimed complete.

`Signature Function` is a cross-Genre Tag. Current members are Fatigue, Curiosity, Greed, and Perspective.
...
When addition remains necessary after Subtraction First, investigate `^ → ¢ → % → $` in that order and record why each earlier class is insufficient. Final classification must remain truthful.

Every implementation Candidate is Blueprint-mapped before debate. Donor implementation may be reused; donor architecture, canonical-state authority, mutation authority, activation, and publication authority may not be inherited.
